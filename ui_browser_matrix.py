...
with sync_playwright() as pw:
    name = os.environ.get('REVIEW_BROWSER', 'firefox')
    browser = pw.chromium.launch(headless=True, channel=name) if name in ('chrome', 'msedge') else getattr(pw, name).launch(headless=True)
    for side, port in [('base', 18755), ('head', 18756)]:
        ...
        session = SimpleNamespace(base_url=f'http://127.0.0.1:{port}', access_token=token, refresh_token='')
        ...
        page.screenshot(path=str(ROOT/f'{name}-{side}-initial.png'))
        (ROOT/f'{name}-{side}-body.txt').write_text(page.locator('body').inner_text())
        if side == 'base':
            ...
            destination = ROOT/f'{name}-downloaded-line_plot.png'
            download.save_as(destination)
            assert destination.read_bytes() == source_bytes
            assert len(requests) == 1, requests
        page.screenshot(path=str(ROOT/f'{name}-{side}-chat.png'))
        reports.append({'side':side,'expected':EXPECT,'browser':name, 'version':browser.version,'dimensions':dimensions,'sandbox_responses':requests,'download_identical':side=='head','page_errors':errors,'plot_sha256':hashlib.sha256(source_bytes).hexdigest()})
        assert not errors, errors
        context.close()
    browser.close()
(ROOT/f'{name}-ui-evidence.json').write_text(json.dumps(reports,indent=2))
