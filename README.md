...
Run the scripts with `PYTHONPATH=studio/backend` and the dependencies in `final-requirements.txt`, inside an isolated credential-free environment. Compare the merge base `16408b25fdef12133be49e117cdd5f9a3382cb3a`, original head `6e2f3da93999d1726ef6136d68896b658eb6fa73`, and repaired source `444d8775c`.
...
`merge-tests.log` runs `test_sandbox_tools.py` on the reviewed prospective merge tree `3023319c7`.
...
The proxy-selection probe records both direct select_proxy and selection after Requests session merging. None keys are removed by that merge and can expose a fallback proxy; direct selection alone does not describe requests.get behavior.
