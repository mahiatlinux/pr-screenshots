...
import ts from "/task/fix/studio/frontend/node_modules/typescript/lib/typescript.js";
import { readFileSync } from "node:fs";
const readSrc = (p: string) => readFileSync("/task/fix/studio/frontend/src/"+p,"utf8");
import {
  localPromptQueueModelBoundary,
  shouldAbortPendingQueueForModelBoundary,
  shouldAbortPendingQueueForSettingsChange,
} from "/task/fix/studio/frontend/src/features/chat/utils/prompt-queue-model-boundary.ts";
import { snapshotQueuedChatRunSettings } from "/task/fix/studio/frontend/src/features/chat/utils/queued-chat-run-settings.ts";
import { reorderPromptQueueItems } from "/task/fix/studio/frontend/src/features/chat/utils/prompt-queue-reorder.ts";
import { steeringInsertionIndex } from "/task/fix/studio/frontend/src/features/chat/utils/composer-preferences.ts";
import { chatModelLifecycleGate } from "/task/fix/studio/frontend/src/features/chat/utils/model-lifecycle-gate.ts";
import { parseExternalModelId } from "/task/fix/studio/frontend/src/features/chat/external-providers.ts";
import {
  planUserPromptQueueStop,
  userStopTargetCancelMode,
} from "/task/fix/studio/frontend/src/features/chat/utils/prompt-queue-user-stop.ts";
