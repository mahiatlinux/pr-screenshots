...
const browser=await chromium.launch({executablePath:path.join(root,'tools/chrome/chrome-linux64/chrome'),headless:true,args:['--no-sandbox']});
const facts={browser:browser.version(),viewport:{width:1400,height:1000},cases:fs.existsSync('artifacts/ui-facts.json')?JSON.parse(fs.readFileSync('artifacts/ui-facts.json','utf8')).cases.filter(c=>!process.argv.slice(2).includes(c.side)):[]};
...
for(const side of sides){
 ...
 for(const [slug,model,levels]of [
  ...
  if(['deepseek','gemini','flash'].includes(slug))await page.screenshot({path:`artifacts/${side}-${slug}-reasoning.png`});
  if(expectLadder){
  ...
  if(slug==='deepseek'||slug==='gpt51'){
   ...
   if(slug==='deepseek')await page.screenshot({path:`artifacts/${side}-image.png`});
  }
  if(slug==='deepseek'){
   ...
   await page.screenshot({path:`artifacts/${side}-tokens.png`});
  }
  facts.cases.push(record);console.log(JSON.stringify(record));
  fs.writeFileSync('artifacts/ui-facts.json',JSON.stringify(facts,null,2));
  await context.close();
 }
}
await browser.close();
