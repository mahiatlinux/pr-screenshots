...
test("legacy flat threads preserve chronological order", async () => {
  const flat = storedMessages([["u",null,"user","Question"],["a",null,"assistant","Answer"]]);
  assert.deepEqual(await shareGptConversations(flat), [{from:"human",value:"Question"},{from:"gpt",value:"Answer"}]);
});
test("empty live branches export no old messages", async () => {
  const result = await shareGptExport(regenerated, []);
  assert.deepEqual(result.downloads, []);
  assert.equal(result.bulk, null);
});
test("inactive threads fall back without reading a different view", async () => {
  const unregister=liveThreadHead.registerLiveThreadView({threadListItem:()=>({getState:()=>({remoteId:"different"})}),thread:()=>{throw new Error("must not be read");}});
  try { assert.deepEqual(await shareGptConversations(regenerated),[{from:"human",value:"Name one fruit."},{from:"gpt",value:"Apples!"}]); }
  finally { unregister(); }
});
test("a torn down view does not hide another matching view", async () => {
  const unregister=liveThreadHead.registerLiveThreadView({threadListItem:()=>{throw new Error("disposed");},thread:()=>{throw new Error("disposed");}});
  try { assert.deepEqual(await shareGptConversations(regenerated,["u1","a1"]),[{from:"human",value:"Name one fruit."},{from:"gpt",value:"Apples."}]); }
  finally { unregister(); }
});
test("selected multi-turn branch excludes both sibling subtrees", async () => {
  const rows=storedMessages([["u",null,"user","Q1"],["a","u","assistant","A1"],["v","a","user","Q2"],["b","v","assistant","A2"],["c","v","assistant","retry"],["w","a","user","edited Q2"],["d","w","assistant","edited A2"]]);
  assert.deepEqual(await shareGptConversations(rows,["u","a","v","b"]),[{from:"human",value:"Q1"},{from:"gpt",value:"A1"},{from:"human",value:"Q2"},{from:"gpt",value:"A2"}]);
});
