...
code = f"import fabric,paramiko; fabric.Connection('approved.example', config=fabric.Config(lazy=True), connect_timeout=1, connect_kwargs={{'sock':paramiko.ProxyCommand({command!r})}}).open()"
result = _python_exec(code, session_id=session, timeout=5)
print(json.dumps({'policy_result':result,'connections':len(connections)},indent=2))
assert 'Blocked:' in result and len(connections) == 0
