...
import ts from "/task/upstream/studio/frontend/node_modules/typescript/lib/typescript.js";
import { readFileSync } from "node:fs";
const readSrc = (p: string) => readFileSync("/task/upstream/studio/frontend/src/"+p,"utf8");
import {
  localPromptQueueModelBoundary,
  shouldAbortPendingQueueForModelBoundary,
  shouldAbortPendingQueueForSettingsChange,
} from "/task/upstream/studio/frontend/src/features/chat/utils/prompt-queue-model-boundary.ts";
import { snapshotQueuedChatRunSettings } from "/task/upstream/studio/frontend/src/features/chat/utils/queued-chat-run-settings.ts";
import { reorderPromptQueueItems } from "/task/upstream/studio/frontend/src/features/chat/utils/prompt-queue-reorder.ts";
import { steeringInsertionIndex } from "/task/head/studio/frontend/src/features/chat/utils/composer-preferences.ts";
import { chatModelLifecycleGate } from "/task/upstream/studio/frontend/src/features/chat/utils/model-lifecycle-gate.ts";
import { parseExternalModelId } from "/task/upstream/studio/frontend/src/features/chat/external-providers.ts";
import {
  planUserPromptQueueStop,
  userStopTargetCancelMode,
} from "/task/upstream/studio/frontend/src/features/chat/utils/prompt-queue-user-stop.ts";
...
function world() {
  ...
  const engine = new Function(
    ...Object.keys(deps),
    `${js}\nreturn {startPromptQueue, dispatchQueuedPrompt, isPromptQueueRunReadyToDispatch, handlePromptQueueRunState, handlePromptQueueRunFailed, resumePromptQueueRun};`,
