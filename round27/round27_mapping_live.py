...
code="import paramiko; clients={'prod':paramiko.SSHClient()}; clients.get('prod').set_missing_host_key_policy(paramiko.AutoAddPolicy()); clients.get('prod').connect(hostname='127.0.0.2',port=22222,username='review',password='review',allow_agent=False,look_for_keys=False); print(clients.get('prod').exec_command('uptime')[1].read().decode()); clients.get('prod').close()"
result=_python_exec(code,session_id=session,timeout=5)
print(json.dumps({'result':result,'connections':len(connections)}),flush=True)
