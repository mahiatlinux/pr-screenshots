...
with http.server.HTTPServer(('127.0.0.1', 0), Handler) as origin, http.server.HTTPServer(('127.0.0.1', 0), Handler) as proxy:
    ...
    cases.update({
        'unrelated_method_name': f"import requests\ns=requests.Session()\ns.trust_env=False\ndef configure():\n    s.proxies={{'http':'http://127.0.0.1:{proxy.server_port}'}}\nclass Other:\n    def configure(self): pass\nOther().configure()\nresponse=s.get('http://pypi.org:{origin.server_port}/',timeout=2)",
        'shadowed_helper_name': f"import requests\ns=requests.Session()\ns.trust_env=False\ndef configure():\n    s.proxies={{'http':'http://127.0.0.1:{proxy.server_port}'}}\ndef run():\n    def configure(): pass\n    configure()\nrun()\nresponse=s.get('http://pypi.org:{origin.server_port}/',timeout=2)",
        'unused_class_method': f"import requests\ns=requests.Session()\ns.trust_env=False\nclass Unused:\n    def configure(self):\n        s.proxies={{'http':'http://127.0.0.1:{proxy.server_port}'}}\nresponse=s.get('http://pypi.org:{origin.server_port}/',timeout=2)",
        'called_class_method': f"import requests\ns=requests.Session()\ns.trust_env=False\nclass Used:\n    def configure(self):\n        s.proxies={{'http':'http://127.0.0.1:{proxy.server_port}'}}\nUsed().configure()\nresponse=s.get('http://pypi.org:{origin.server_port}/',timeout=2)",
