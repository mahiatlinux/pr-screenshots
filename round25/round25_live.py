...
watch_command='TERM=xterm timeout 1s watch -t -n 60 '+command
watch_result=_bash_exec(watch_command,session_id=session,timeout=5)
print('watch before:',repr(watch_result),'connections:',len(connections),flush=True)
assert 'Blocked' in watch_result and not connections
...
pty_code=f"import pty; pty.spawn({shlex.split(command)!r})"
reflective_code=alias_factory_code.replace('Factory=paramiko.SSHClient', "Factory=getattr(paramiko,'SSHClient')").replace("c.connect('127.0.0.2',", "getattr(c,'connect')(hostname='127.0.0.2',")
for name, new_code in [('pty',pty_code),('reflective',reflective_code)]:
 result=_python_exec(new_code,session_id=session,timeout=10)
 print(name,'before:',result,'connections:',len(connections),flush=True)
indirect_codes.update(pty=pty_code,reflective=reflective_code)
dynamic_import_code=alias_factory_code.replace('import paramiko', 'import importlib\np=importlib.import_module("paramiko")').replace('paramiko.', 'p.').replace("c.connect('127.0.0.2',", "c.connect(hostname='127.0.0.2',")
class_client_code=alias_factory_code.replace('c=Factory()', 'class Pool: client=Factory()').replace('c.', 'Pool.client.').replace("Pool.client.connect('127.0.0.2',", "Pool.client.connect(hostname='127.0.0.2',")
for name,new_code in [('dynamic_import',dynamic_import_code),('class_client',class_client_code)]:
 result=_python_exec(new_code,session_id=session,timeout=10)
 print(name,'before:',result,'connections:',len(connections),flush=True)
indirect_codes.update(dynamic_import=dynamic_import_code,class_client=class_client_code)
partial_code=alias_factory_code.replace('import paramiko', 'import paramiko,functools').replace('Factory=paramiko.SSHClient','Factory=functools.partial(paramiko.SSHClient)').replace("c.connect('127.0.0.2',", "connect=functools.partial(c.connect,hostname='127.0.0.2')\nconnect(")
result=_python_exec(partial_code,session_id=session,timeout=10)
print('partial before:',result,'connections:',len(connections),flush=True)
indirect_codes['partial']=partial_code
parameter_code=alias_factory_code.replace("c.connect('127.0.0.2',", "def deploy(client):\n client.connect(hostname='127.0.0.2',").replace("_, out, _", "deploy(c)\n_, out, _")
result=_python_exec(parameter_code,session_id=session,timeout=10)
print('parameter before:',result,'connections:',len(connections),flush=True)
expanded_command='cmd=${PR10642_UNSET:-ssh}; "$cmd" '+command.removeprefix('ssh ')
expanded_before=_bash_exec(expanded_command,session_id=session,timeout=10)
print('expanded command before:',expanded_before,'connections:',len(connections),flush=True)
indirect_codes['helper_parameter']=parameter_code
method_code=parameter_code.replace('def deploy(client):\n client.connect', 'class Deployer:\n def deploy(self,client):\n  client.connect').replace('deploy(c)', 'Deployer().deploy(c)')
method_before=_python_exec(method_code,session_id=session,timeout=10)
print('method before:',method_before,'connections:',len(connections),flush=True)
indirect_codes['method_parameter']=method_code
constructor_code=method_code.replace('def deploy(self,client):','def __init__(self,client):').replace('Deployer().deploy(c)','Deployer(c)')
constructor_before=_python_exec(constructor_code,session_id=session,timeout=10)
print('constructor before:',constructor_before,'connections:',len(connections),flush=True)
indirect_codes['constructor_parameter']=constructor_code
loop_code=parameter_code.replace('def deploy(client):', 'for client in [c]:').replace('deploy(c)\n','')
loop_before=_python_exec(loop_code,session_id=session,timeout=10)
print('loop before:',loop_before,'connections:',len(connections),flush=True)
indirect_codes['loop_client']=loop_code
conditional_code=alias_factory_code.replace('c=Factory()', 'c=Factory() if True else None').replace("c.connect('127.0.0.2',", "c.connect(hostname='127.0.0.2',")
boolean_code=conditional_code.replace('Factory() if True else None','Factory() or None')
for name,branch_code in [('conditional_client',conditional_code),('boolean_client',boolean_code)]:
 result=_python_exec(branch_code,session_id=session,timeout=10)
 print(name,'before:',result,'connections:',len(connections),flush=True)
 indirect_codes[name]=branch_code
instance_code=alias_factory_code.replace('c=Factory()', 'class Pool:\n def __init__(self): self.client=Factory()\npool=Pool()').replace('c.', 'pool.client.').replace("pool.client.connect('127.0.0.2',", "pool.client.connect(hostname='127.0.0.2',")
inline_instance_code=f"import paramiko\nclass Pool:\n def __init__(self):\n  self.client=paramiko.SSHClient()\n  self.client.set_missing_host_key_policy(paramiko.AutoAddPolicy())\nPool().client.connect(hostname='127.0.0.2',port={port},username='review',password='disposable',allow_agent=False,look_for_keys=False)\nprint('pr10642-live-ssh-ok')"
for name,instance_probe in [('instance_client',instance_code),('inline_instance_client',inline_instance_code)]:
 result=_python_exec(instance_probe,session_id=session,timeout=10)
 print(name,'before:',result,'connections:',len(connections),flush=True)
 indirect_codes[name]=instance_probe
iterator_code=alias_factory_code.replace('c=Factory()', 'c=next(iter([Factory()]))').replace("c.connect('127.0.0.2',", "c.connect(hostname='127.0.0.2',")
reversed_code=iterator_code.replace('iter([Factory()])','reversed([Factory()])')
for name,iterator_probe in [('iterator_client',iterator_code),('reversed_client',reversed_code)]:
 result=_python_exec(iterator_probe,session_id=session,timeout=10)
 print(name,'before:',result,'connections:',len(connections),flush=True)
 indirect_codes[name]=iterator_probe
selection_code=alias_factory_code.replace('c=Factory()', 'c=min([Factory()])').replace("c.connect('127.0.0.2',", "c.connect(hostname='127.0.0.2',")
result=_python_exec(selection_code,session_id=session,timeout=10)
print('min selection before:',result,'connections:',len(connections),flush=True)
...
approve_hosts(session, ['127.0.0.2'])
selection_after=_python_exec(selection_code,session_id=session,timeout=10)
print('min selection after approval:',selection_after,'connections:',len(connections),flush=True)
assert 'Blocked' in selection_after and not connections
watch_after=_bash_exec(watch_command,session_id=session,timeout=5)
print('watch after approval:',repr(watch_after),'connections:',len(connections),flush=True)
assert 'pr10642-live-ssh-ok' in watch_after and len(connections)==1
