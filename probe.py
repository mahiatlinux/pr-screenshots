...
mods = {side:[load(side,i) for i in range(2)] for side in ['base','head','merge']}
...
for cols in schemas:
 for lens in itertools.product(lengths,repeat=len(cols)):
  ...
  for idx in range(2):
   old=mods['base'][idx].detect_custom_format_heuristic([row]); new=mods['head'][idx].detect_custom_format_heuristic([row]); merged=mods['merge'][idx].detect_custom_format_heuristic([row]); assert new==merged
   if old!=new: differences.append({'columns':cols,'lengths':lens,'module':idx,'base':old,'head':new})
...
(root/'artifacts/probe.json').write_text(json.dumps(result,indent=2)); print(json.dumps(result,indent=2))
